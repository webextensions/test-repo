...
var stringify = require('./json-stable-stringify/json-stable-stringify.js');

var generateSerializedStakeHolders = async function () {
    var packageJson;
    ...
    var strNvmrc;
    try { strNvmrc = await readFile(path.resolve(__dirname, '../../../.nvmrc'), 'utf8'); } catch (e) { /* do nothing */ }

    var strNpmrc;
    try { strNpmrc = await readFile(path.resolve(__dirname, '../../../.npmrc'), 'utf8'); } catch (e) { /* do nothing */ }
    ...
    if (strNvmrc) {
        stakeholders.strNvmrc = strNvmrc;
    }

    var serializedStakeholders = stringify(stakeholders);
...
var generateHash = async function () {
    var serializedStakeholders = await generateSerializedStakeHolders(),
        hash = getSHA512Hash(serializedStakeholders);
    return hash;
};

var generateShortHash = async function () {
