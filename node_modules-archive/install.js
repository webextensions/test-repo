...
var restoreNode_ModulesFromBackup = async function (nodeModulesBackupFilePath) {
    ...
    // https://stackoverflow.com/questions/38273253/using-two-commands-using-pipe-with-spawn/38273497#38273497
    await spawnWrtProjectRoot('sh', ['-c', 'mkdir -p node_modules-archive/' + directoryName]);
    await spawnWrtProjectRoot('sh', ['-c', 'tar -xzf ' + nodeModulesBackupFilePath + ' --directory node_modules-archive/' + directoryName]);
    await spawnWrtProjectRoot('sh', ['-c', 'rm -rf node_modules']);
    await spawnWrtProjectRoot('sh', ['-c', 'mv node_modules-archive/' + directoryName + '/node_modules ./']);
    await spawnWrtProjectRoot('sh', ['-c', 'rm -rf node_modules-archive/' + directoryName]);
};

var spawnWrtProjectRoot = async function (app, params) {
    var cwd = path.resolve(__dirname, '..');
    var exitCode = await spawn(app, params, { cwd });
...
var main = async function () {
    ...
    var nodeModulesBackupFilePath = path.resolve(__dirname, '..', 'node_modules-archive', 'node_modules-' + stakeholdersHash + '.tar.gz');
