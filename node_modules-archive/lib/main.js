...
var quickInstall = async function (options) {
    ...
    var getFreeDiskSpaceOfPartitionForPath = async function (itemPath) {
        ...
        var output = await exec(
            'sh',
            ['-c', 'df -B1 "' + itemPath + '" | tail -1 | tr -s " " | cut -d" " -f4']
        );
    ...
    var getSizeOfGivenPath = async function (itemPath) {
        ...
        console.log('Exit code: ' + exitCode);
        if (output.err) {
            console.log('Warning: Could not get occupied disk space for the node_modules archive folder');
            warningOccurred = true;
        }
    ...
    var freePartitionSpaceInBytes = await getFreeDiskSpaceOfPartitionForPath(nodeModulesBackupFilePath);
