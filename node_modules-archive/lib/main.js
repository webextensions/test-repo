...
var quickInstall = async function (options) {
    ...
    var getSizeOfGivenPath = async function (itemPath) {
    ...
    if (fs.existsSync(path.dirname(nodeModulesBackupFilePath))) {
        var sizeOccupiedByArchiveFolder = await getSizeOfGivenPath(path.dirname(nodeModulesBackupFilePath));
        if (sizeOccupiedByArchiveFolder === null) {
            warningOccurred = true;
            console.log('\nWarning: The size occupied by the node_modules archive folder is NOT AVAILABLE.\n');
        } else {
            console.log('\nInfo: The size occupied by the node_modules archive folder is ' + humanReadableByteSize(sizeOccupiedByArchiveFolder) + '.\n');
        }
        ...
        if (freePartitionSpaceInBytes === null) {
            ...
            if (freePartitionSpaceInBytes < showDiskSpaceWarningBelow) {
                console.warn('\nWarning: The available disk space for the partition containing node_modules archive folder is ' + humanReadableByteSize(freePartitionSpaceInBytes) + '.\n');
                warningOccurred = true;
            } else {
                console.log('\nInfo: The available disk space for the partition containing node_modules archive folder is ' + humanReadableByteSize(freePartitionSpaceInBytes) + '.\n');
            }
        }
    }
