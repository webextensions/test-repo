...
var quickInstall = async function (options) {
    ...
    var getNodeModulesBackupFilePath = async function (options) {
        var { projectRoot, archiveDirectory } = options;
        var stakeholdersHash = await generateStakeholdersHash({projectRoot}),
            nodeModulesBackupFilePath = path.resolve(projectRoot, archiveDirectory, 'node_modules-' + stakeholdersHash + '.tar.gz');
        return nodeModulesBackupFilePath;
    };

    var nodeModulesBackupFilePath = await getNodeModulesBackupFilePath({projectRoot, archiveDirectory});
    ...
    if (fs.existsSync(path.dirname(nodeModulesBackupFilePath))) {
        ...
        if (sizeOccupiedByArchiveDirectory === null) {
            ...
            console.log('\nWarning: The size occupied by the node_modules archive directory is NOT AVAILABLE.\n');
        } else {
            console.log('\nInfo: The size occupied by the node_modules archive directory is ' + humanReadableByteSize(sizeOccupiedByArchiveDirectory) + '.\n');
        }
        ...
        if (freePartitionSpaceInBytes === null) {
            console.warn('\nWarning: Could not get information about the free disk space for the partition where node_modules archive is expected to be backed up.\n');
            warningOccurred = true;
        } else {
            ...
            if (freePartitionSpaceInBytes < showDiskSpaceWarningBelow) {
                console.warn('\nWarning: The available disk space for the partition containing node_modules archive directory is ' + humanReadableByteSize(freePartitionSpaceInBytes) + '.\n');
                warningOccurred = true;
            } else {
                console.log('\nInfo: The available disk space for the partition containing node_modules archive directory is ' + humanReadableByteSize(freePartitionSpaceInBytes) + '.\n');
            }
        }
    }
        ...
        if (fileExistsRelativeToProjectRoot('package-lock.json')) {
            if (arePackageJsonAndPackageLockJsonInSync(projectRoot)) {
                ...
                if (exitCode === 0) {
                    ...
                    nodeModulesBackupFilePath = await getNodeModulesBackupFilePath({projectRoot, archiveDirectory});
                } else {
                    exitCode = await spawnWrtProjectRoot('npm', ['install', '--no-package-lock']);
                    if (exitCode !== 0) {
                        exitCode = await spawnWrtProjectRoot('npm', ['ci']);
