...
var runNpmInstall = async function (options) {
    ...
    var getNodeModulesBackupFilePath = async function (options) {
        var { projectRoot, archiveFolderWrtProjectRoot } = options;
    ...
    var nodeModulesBackupFilePath = await getNodeModulesBackupFilePath({projectRoot, archiveFolderWrtProjectRoot});
        ...
        if (fileExistsRelativeToProjectRoot('package-lock.json')) {
            if (arePackageJsonAndPackageLockJsonInSync(projectRoot)) {
                ...
                if (exitCode === 0) {
                    ...
                    nodeModulesBackupFilePath = await getNodeModulesBackupFilePath({projectRoot, archiveFolderWrtProjectRoot});
                } else {
                    exitCode = await spawnWrtProjectRoot('npm', ['install', '--no-package-lock']);
                    if (exitCode !== 0) {
                        exitCode = await spawnWrtProjectRoot('npm', ['ci']);
