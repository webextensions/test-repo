    ...
    if (fs.existsSync(nodeModulesBackupFilePath)) {
        ...
        if (fileExistsRelativeToProjectRoot('package-lock.json')) {
            if (arePackageJsonAndPackageLockJsonInSync(projectRoot)) {
                ...
                if (exitCode === 0) {
                    console.log('TODO: Update nodeModulesBackupFilePath since the stakeholders might have changed');
                    console.log('Exiting with error till when this TODO task is not resolved.\nExit code: 1');
                    process.exit(1);
                } else {
                    exitCode = await spawnWrtProjectRoot('npm', ['install', '--no-package-lock']);
                    if (exitCode !== 0) {
